...
node("nodejs"){
    ...
    stage('Clone Git Repo') {
        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
            git branch: 'master', credentialsId: 'github-goitProjects', url: 'git@github.com:goitProjects/kidslike_v2-backend.git'
        }
    }
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                ...
                sh "mkdir -p /root/slave_jenkins/workspace/Build-Kidslike-V2-backend/src/gcp-config"
                //Copy files for Build stage
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-Kidslike-V2-backend/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-Kidslike-V2-backend/src/gcp-config/keys.json"
                sh "cp /root/keys/kidslike-v2-backend/.env /root/slave_jenkins/workspace/Build-Kidslike-V2-backend/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  kidslike-backend.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/kidslike"
            }
        }
    }
